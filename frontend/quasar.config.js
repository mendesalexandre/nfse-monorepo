...
export default defineConfig((ctx) => {
  return {
    ...
    boot: ["fontawesome", "i18n", "axios", "echo", "setDefaults", "components"],

    // https://v2.quasar.dev/quasar-cli-webpack/quasar-config-file#css
    css: ["app.scss", "tinymce.scss"],
    ...
    devServer: {
      ...
      // Proxy para Sanctum SPA Authentication
      proxy: [
        {
          context: ["/api", "/sanctum", "/login", "/logout", "/register", "/forgot-password"],
          target: process.env.BACKEND_URL || "http://nginx",
          changeOrigin: true,
        },
      ],
    },
